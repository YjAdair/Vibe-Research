...
## 5.6 市场栏目与题材轮动（迁入模块纪律）

来自热点-题材轮动收口经验；完整说明见 [`docs/theme-rotation/`](docs/theme-rotation/README.md)。

- 对齐目标站的是**功能与计算语义**，不是爬其接口结果入库；免费公开数据优先，付费未配置密钥与预算前不得自动调用。
- 市场数据四条铁律：缺失 ≠ 0；旧行情 ≠ 停牌；采集时间 ≠ 行情时间；未来成分快照不可用于过去。
- GET 只读已发布事实；历史缺数标 missing，不借邻日、不在读路径偷偷回填；盘中 refresh / 实时价须显式例外并写进契约。
- 分类宇宙不静默映射（如东财 BK ≠ 目标 801）；HTTP 200 / 单测通过 ≠ 年度逐指标覆盖完成。
- 无核验 OHLC 时只许显式日净值序列，禁止用收盘价拼假蜡烛。
- 按模块推进；同一问题最多尝试 3 次，达上限写入待办后换题；遵守当轮时间与额度，不为用满时长扩 scope。
